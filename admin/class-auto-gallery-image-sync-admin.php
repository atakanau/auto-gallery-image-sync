...
class Auto_Gallery_And_Image_Sync_Admin {
	...
	public function init_options_page() {
		...
		add_submenu_page(
			...
			__('Auto Gallery And Image Sync',$this->plugin_name), // page <title>Title</title>
 ...
 */
	...
	public function get_columns() {
		return array(
			...
			'syncid'	=> __( 'Sync by id', $this->plugin_name ),
			'syncsku'	=> __( 'Sync by SKU', $this->plugin_name ),
		);
	}
...
#region product sku
	...
	public function get_matched_images($img_name_prefix) {
		...
		$result = array();
		$sql_text = "SELECT post_name FROM $wpdb->posts WHERE " // ID, 
		."`post_type` = 'attachment' "
		."AND post_mime_type LIKE %s "
		."AND post_name REGEXP '^(".$img_name_prefix."-)[[:digit:]]+$' "
		."ORDER BY `post_name` ASC"
		;

		// $result[] = $wpdb->remove_placeholder_escape($wpdb->prepare( $sql_text , "image%" ) );
		$images = $wpdb->get_results( $wpdb->prepare( $sql_text , "image%" )  ); // , ARRAY_N
		if( count( $images ) != 0 ){
	...
	public function get_gallery_count($post_id) {
		...
		$result = 0;
		$sql_text = "SELECT meta_value FROM $wpdb->postmeta WHERE "
		."`post_id` = %d "
		."AND `meta_key` = '_product_image_gallery' "
		;

		$images = $wpdb->get_results( $wpdb->prepare( $sql_text , $post_id ) ); // , ARRAY_N
		if( count( $images ) != 0 ){
	...
	protected function bulk_sync_by_id( $id_arr ){
		...
		$result = array(0,0);
		$sql_text = "INSERT INTO $wpdb->postmeta (`post_id`, `meta_key`, `meta_value`) "
...
;
		$sql_text = $wpdb->prepare( $sql_text , "image%", implode(',',$id_arr) );
		$result[0] = $wpdb->query($sql_text);
